...
export const ThumbnailGeneratorPage = () => {
    ...
    const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files) {
            ...
            if (images.length + newImages.length > 6) {
                setError(t('thumbnailGenerator.error.maxImages'));
                return;
            }
    ...
    const handleGenerate = async () => {
        if (!description) {
            setError(t('thumbnailGenerator.error.descriptionRequired'));
            return;
        }
            ...
            <div className="max-w-6xl mx-auto">
                <div className="mb-8 text-center">
                    <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">
                        {t('tools.thumbnailGenerator.title')}
                    </h1>
                    <p className="text-gray-600 dark:text-gray-400 max-w-2xl mx-auto">
                        {t('tools.thumbnailGenerator.description')}
                    </p>
                </div>
                    ...
                    <div className="space-y-6 bg-white dark:bg-surface-paper p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 order-2 lg:order-1">
                        <div>
                            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                {t('thumbnailGenerator.videoDescription')}
                            </label>
                            <textarea
                                value={description}
                                ...
                                placeholder={t('thumbnailGenerator.videoDescriptionPlaceholder')}
                            />
                        </div>

                        <div>
                            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                {t('thumbnailGenerator.visualDescription')}
                            </label>
                            <textarea
                                ...
                                placeholder={t('thumbnailGenerator.visualDescriptionPlaceholder')}
                            />
                            <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                                {t('thumbnailGenerator.visualDescriptionHint')}
                            </p>
                        </div>

                        <div>
                            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                {t('thumbnailGenerator.textLanguage')}
                            </label>
                            <input
                                type="text"
                                value={language}
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                {t('thumbnailGenerator.referenceImages')}
                            </label>
                            <div className="grid grid-cols-3 gap-3 mb-4">
                                ...
                                {images.length < 6 && (
                                    <label className="aspect-square rounded-lg border-2 border-dashed border-gray-300 dark:border-gray-700 flex flex-col items-center justify-center cursor-pointer hover:border-primary dark:hover:border-primary transition-colors bg-gray-50 dark:bg-surface-hover">
                                        ...
                                        <span className="text-xs text-gray-500">{t('thumbnailGenerator.upload')}</span>
                                        <input
                                            type="file"
                                            accept="image/*"
                                            multiple
                        ...
                        {result ? (
                            ...
                            <div className="flex-1 flex flex-col items-center justify-center text-gray-400 dark:text-gray-600">
                                ...
                                <p className="text-center">
                                    {loading ? t('thumbnailGenerator.preview.loading') : t('thumbnailGenerator.preview.placeholder')}
                                </p>
                            </div>
                        )}
                    </div>
